...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, 
                        id2label: Dict, title: str, filename: str):
   ...
   cm_norm = np.divide(cm.astype('float'), 
   ...
   sns.heatmap(cm_norm, annot=True, fmt='.2f', 
               ...
               annot_kws={"size": 9}, cbar_kws={'label': 'Recall'})
...
def plot_entropy_separation(correct_entropies: np.ndarray, 
                        incorrect_entropies: np.ndarray, 
                        threshold: float, filename: str):
   plt.figure(figsize=(10, 6))
   
   sns.kdeplot(correct_entropies, fill=True, color='green', 
               label='Correct', clip=(0, 2.0))
   sns.kdeplot(incorrect_entropies, fill=True, color='red', 
               label='Incorrect', clip=(0, 2.0))
...
def plot_model_comparison(baseline_report: Dict, hybrid_report: Dict, 
                        filename: str):
   labels = [k for k in baseline_report.keys() 
            if k not in ['accuracy', 'macro avg', 'weighted avg'] 
            and k in hybrid_report]
   ...
   plt.bar(x - width/2, base_f1, width, label='Baseline', 
         color='lightgray', edgecolor='black')
   plt.bar(x + width/2, hybrid_f1, width, label='Hybrid (Ours)', 
...
def process_sample(sample: Dict, model, tokenizer, router, 
   ...
   if "offset_mapping" in sample and sample["offset_mapping"] is not None:
      ...
      tokenized_ref = tokenizer(full_text, return_offsets_mapping=True, 
   ...
   for j in range(len(labels)):
      ...
      # FIXED: Skip special tokens by ID instead of string comparison
      ...
      # FIXED: Proper offset tracking
      char_start, char_end = 0, 0
      if offset_idx < len(offsets):
         char_start, char_end = offsets[offset_idx]
         offset_idx += 1
      else:
         logger.warning(f"Offset index {offset_idx} exceeds offsets length {len(offsets)}")

      should_trigger = (use_llm and 
                        ent > THRESHOLD_ENTROPY and 
                        conf < THRESHOLD_CONF)
      ...
      if should_trigger:
         ...
         try:
               ...
               # FIXED: Log errors instead of silencing
   ...
   return (baseline_preds_list, hybrid_preds_list, true_labels_list,
         correct_entropies_list, incorrect_entropies_list,
         llm_intervention_count, llm_correction_count)
...
def evaluate(llm_routing: bool = True):
   ...
   for i in tqdm(range(len(eval_dataset)), desc="Processing"):
      ...
      results = process_sample(
         sample, model, tokenizer, router, 
         id2label, label2id, device, llm_routing
      )
      ...
      # FIXED: Clear CUDA cache periodically
   ...
   logger.info(f"LLM Correction Yield:   {llm_correction_total}/{llm_intervention_total} triggers changed label")
   logger.info("="*60)
   ...
   plot_confusion_matrix(true_labels, baseline_preds, id2label, 
                        "Baseline Confusion Matrix", 
                        f"{OUTPUT_DIR}/cm_baseline.png")
   plot_confusion_matrix(true_labels, hybrid_preds, id2label, 
                        "Hybrid Confusion Matrix", 
                        f"{OUTPUT_DIR}/cm_hybrid.png")
   plot_entropy_separation(correct_entropies, incorrect_entropies, 
                        THRESHOLD_ENTROPY,
                        f"{OUTPUT_DIR}/entropy_calibration.png")
   plot_model_comparison(base_report_dict, hybrid_report_dict, 
                        f"{OUTPUT_DIR}/f1_comparison.png")
