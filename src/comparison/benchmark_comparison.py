...
# Schema Unificato
UNIFIED_SCHEMA = {
   "GIVENNAME": "PERSON", "SURNAME": "PERSON", "PERSON": "PERSON", "PER": "PERSON",
   "CITY": "LOCATION", "STREET": "LOCATION", "ZIPCODE": "LOCATION",
   "BUILDINGNUM": "LOCATION", "LOCATION": "LOCATION", "GPE": "LOCATION",
   ...
   "IDCARDNUM": "GOV_ID", "PASSPORTNUM": "GOV_ID", "DRIVERLICENSENUM": "GOV_ID",
   "TAXNUM": "GOV_ID", "SOCIALNUM": "GOV_ID", "SSN": "GOV_ID",
   "CREDITCARDNUMBER": "CREDIT_CARD", "CREDIT_CARD": "CREDIT_CARD",
   "AGE": "AGE", "SEX": "GENDER", "GENDER": "GENDER", "TITLE": "TITLE"
}
...
def set_academic_style():
   """Configura uno stile accademico moderno e professionale."""
...
def get_model_palette(models):
   """Palette di colori professionale e distinguibile."""
   unique_models = sorted(list(set(models)))
   # Palette personalizzata con colori bilanciati e professionali
   colors = ['#2E86AB', '#A23B72', '#F18F01', '#C73E1D', '#6A994E', 
            '#BC4B51', '#8E7DBE', '#5C8001']
   return dict(zip(unique_models, colors[:len(unique_models)]))

def plot_main_metrics(df_metrics, output_dir):
   """Grafici principali: F1-Score e Latency con layout ottimizzato."""
   ...
   # F1 Score - ordinato per valore
   df_f1 = df_metrics.sort_values('F1-Score')
   colors_f1 = [model_colors[m] for m in df_f1['Model']]
   
   bars1 = ax1.barh(range(len(df_f1)), df_f1['F1-Score'], 
                  color=colors_f1, edgecolor='#2C2C2C', 
                  linewidth=1.2, height=0.65, alpha=0.9)
   ...
   for i, (bar, val) in enumerate(zip(bars2, df_lat['Latency (ms)'])):
      ax2.text(val + (max_lat * 0.02), i, f'{val:.1f}', 
               va='center', ha='left', fontsize=10, fontweight='bold', color='#333333')
   
   # Rimuovi spine superiori e destre per un look più pulito
   for ax in [ax1, ax2]:
...
def plot_efficiency_frontier(df_metrics, output_dir):
   ...
   # Palette colori vibrante ma professionale
   colors_list = ['#2E86AB', '#A23B72', '#F18F01', '#C73E1D', 
                  '#6A994E', '#BC4B51', '#8E7DBE', '#5C8001']
   markers = ['o', 's', 'D', '^', 'v', 'P', 'X', '*']
   ...
   for i, (idx, row) in enumerate(df_sorted.iterrows()):
      ...
      # Scatter con dimensione ottimale
      ax.scatter(x, y, s=350, c=color, marker=marker,
      ...
      ax.annotate(row['Model'], xy=(x, y), xytext=(0, y_offset),
   ...
   ax.set_xscale('log')
   ax.set_ylim(max(0, df_sorted['F1-Score'].min() - 0.08), 
               min(1.05, df_sorted['F1-Score'].max() + 0.08))
   
   ax.set_title('Efficiency Frontier: Latency vs Performance', 
   ...
   ax.legend(loc='best', frameon=True, facecolor='white', 
            framealpha=0.95, edgecolor='#CCCCCC', fontsize=9.5,
...
def plot_entity_comparison(results, output_dir):
   ...
   sns.barplot(data=df_ent, x='Entity', y='F1-Score', hue='Model', 
               palette=palette, edgecolor='#2C2C2C', linewidth=1.2,
               alpha=0.9, ax=ax)
   
   ax.set_title('Performance Breakdown by Entity Type', 
               fontsize=15, fontweight='bold', pad=15)
   ax.set_xlabel('Entity Type', fontsize=13)
   ax.set_ylabel('F1-Score', fontsize=13)
   ax.set_ylim(0, 1.05)
   
   # Legenda esterna per chiarezza
...
def plot_results(results, output_dir="evaluation_results"):
   """Pipeline principale di generazione grafici."""
   set_academic_style()
   ...
   for res in results:
      ...
      y_pred_no_o = [res['y_pred'][i] for i, y in enumerate(res['y_true']) if y != 'O']
   ...
   print(f"\n📊 Generating professional academic visualizations (DPI: 300)...")
   ...
   for res in results:
      plot_confusion_matrix_single(res['y_true'], res['y_pred'], 
                                    res['model'], output_dir)
   
   df_metrics.to_csv(f"{output_dir}/metrics_summary.csv", index=False)
   print(f"  ✓ Publication-ready visualizations saved in: {os.path.abspath(output_dir)}")
...
class NerGuard(ModelWrapper):
   def __init__(self, model_path, enable_llm=True):
      ...
      # SOGLIE COME NEL CODICE ORIGINALE (entropy_inference.py)
      self.thresh_ent = 0.583  # THRESHOLD originale
      self.thresh_conf = 0.787  # THRESHOLD_CONF originale
      ...
      self.high_entropy_tokens = 0  # Quanti token superano la soglia
      
      if self.enable_llm:
         print(f"    LLM enabled with thresholds: entropy > {self.thresh_ent}, conf < {self.thresh_conf}")
   ...
   def predict(self, sample, tokenizer, id2label):
      ...
      with torch.no_grad():
         logits = self.model(input_ids_tensor, attention_mask=attention_mask).logits[0]
      ...
      for i, lbl_id in enumerate(labels):
         ...
         # LLM disambiguation - FIXED con offset mapping
         if self.enable_llm and entropy[i] > self.thresh_ent and conf[i] < self.thresh_conf:
               ...
               try:
                  ...
                  if i < len(offset_mapping):
                     ...
                     # DEBUG: primi 3 esempi
                     if self.llm_calls <= 3:
                           print(f"\n  [LLM DEBUG #{self.llm_calls}]")
                           print(f"    Token: '{tokens[i]}' | Entropy: {entropy[i]:.3f} | Conf: {conf[i]:.3f}")
                           print(f"    Pred: {raw_pred} | Prev: {prev_pred}")
                           print(f"    Chars: {char_start}-{char_end} in '{text[max(0,char_start-20):char_end+20]}'")
                     ...
                     res = self.router.disambiguate(
                           ...
                           char_end=char_end,
                           current_pred=raw_pred,  # AGGIUNTO
                     ...
                     if self.llm_calls <= 3:
                           print(f"    LLM Result: {new_label} (is_pii: {res.get('is_pii')})")
...
class ExternalWrapper(ModelWrapper):
   def __init__(self, engine_type):
      ...
      if engine_type == "spacy" and SPACY_AVAILABLE:
         ...
         self.labels = ["person", "city", "street", "phone", "email", "date", 
                        "id", "passport", "age", "gender", "job"]
         self.map = {
               "person":"PERSON", "city":"LOCATION", "street":"LOCATION", 
               "phone":"PHONE_NUMBER", "email":"EMAIL_ADDRESS", "date":"DATE_TIME", 
               "id":"GOV_ID", "passport":"GOV_ID", "age":"AGE", 
               "gender":"GENDER", "job":"TITLE"
         }
   ...
   def _get_spans(self, text):
      ...
      if self.type == "spacy":
         doc = self.nlp(text)
         spans = [EntitySpan(ent.label_, ent.start_char, ent.end_char, ent.text) 
                  for ent in doc.ents]
      elif self.type == "presidio":
         res = self.analyzer.analyze(text=text, language='en')
         spans = [EntitySpan(r.entity_type, r.start, r.end, text[r.start:r.end]) 
                  for r in res]
      elif self.type == "gliner":
         preds = self.model.predict_entities(text, self.labels, threshold=0.5)
         spans = [EntitySpan(self.map.get(p['label'], "O"), p['start'], p['end'], p['text']) 
                  for p in preds]
      return spans
         ...
         if i < len(offset_mapping):
               ...
               for span in spans:
                  ...
                  if overlap_end > overlap_start:
                     overlap_ratio = (overlap_end - overlap_start) / (end_char - start_char)
...
def run_benchmark():
   ...
   if LLM_AVAILABLE:
      ...
      print("⚠️  Skipping NerGuard Hybrid (LLMRouter not available)\n")
   
   if SPACY_AVAILABLE: models.append(ExternalWrapper("spacy"))
   if PRESIDIO_AVAILABLE: models.append(ExternalWrapper("presidio"))
   if GLINER_AVAILABLE: models.append(ExternalWrapper("gliner"))
   ...
   for model in models:
      ...
      if isinstance(model, NerGuard) and model.enable_llm:
         total_tokens = len([l for sample in dataset for l in sample["labels"] if l != -100])
