...
# --- MAPPING CONSTANTS ---
NVIDIA_TO_MODEL_MAP = {
   "first_name": "GIVENNAME", "last_name": "SURNAME", "middle_name": "GIVENNAME",
   "name": "GIVENNAME", "user_name": "GIVENNAME",
   "email": "EMAIL",
   "phone_number": "TELEPHONENUM", "cell_phone": "TELEPHONENUM", "fax_number": "TELEPHONENUM",
   "city": "CITY", "zipcode": "ZIPCODE", "postcode": "ZIPCODE",
   "ssn": "SOCIALNUM", "social_security_number": "SOCIALNUM",
   "tax_id": "TAXNUM",
   "driver_license": "DRIVERLICENSENUM",
   "credit_debit_card": "CREDITCARDNUMBER",
   "date": "DATE", "date_of_birth": "DATE", "time": "TIME",
   "age": "AGE", "gender": "GENDER",
   "street_address": "STREET",
   "company_name": "O", "organization": "O", "url": "O", "ip_address": "O",
   "country": "O", "state": "O", "national_id": "O", "passport_number": "O",
}
...
class HybridResult:
   ...
   # Tracciamento dettagliato per analisi
   confidence_scores: List[float] = field(default_factory=list)
   entropy_scores: List[float] = field(default_factory=list)
   llm_interventions: List[Dict] = field(default_factory=list)
   per_class_stats: Dict[str, Dict] = field(default_factory=lambda: defaultdict(lambda: {
      'total': 0, 'baseline_correct': 0, 'hybrid_correct': 0, 
      'llm_called': 0, 'llm_helped': 0, 'llm_hurt': 0
   }))
...
class HybridEvaluator(PlottingMixin):
   def __init__(self, config: EvalConfig, llm_router: Optional[OptimizedLLMRouter] = None):
      self.config = config
   ...
   def _parse_spans(spans: Any) -> List[Dict]:
      if isinstance(spans, str):
         try: spans = json.loads(spans)
               ...
               try: spans = ast.literal_eval(spans)
               except: return []
   ...
   def evaluate_comparison(self, dataset):
      ...
      logger.info(f"Using thresholds - Confidence: {self.config.THRESHOLD_CONF}, Entropy: {self.config.THRESHOLD_ENTROPY}")
      
      res = HybridResult()
      
      for i, example in tqdm(enumerate(dataset), total=len(dataset), desc="Processing"):
         ...
         if self.llm_router:
               for idx, (token_id, pred_id, conf, ent) in enumerate(zip(input_ids[0], preds_flat, conf_flat, entropy_flat)):
                  ...
                  # Tracciamento statistiche per classe
                  res.per_class_stats[true_label]['total'] += 1
                  if pred_id == labels_flat[idx]:
                     res.per_class_stats[true_label]['baseline_correct'] += 1
                  
                  # Salva confidence ed entropy per analisi
                  res.confidence_scores.append(float(conf))
                  res.entropy_scores.append(float(ent))
                  
                  is_uncertain = (conf < self.config.THRESHOLD_CONF) and (ent > self.config.THRESHOLD_ENTROPY)

                  # Skip LLM processing for 'O' tags (both prediction and true label) to avoid unnecessary API calls
                  if is_uncertain and pred_label != "O" and true_label != "O":
                     ...
                     prev_label = self.mapper.model_id2label[hybrid_preds_flat[idx-1]] if idx > 0 else "O"
                     ...
                     corrected_id = self.mapper.model_label2id.get(corrected_label, 
                                       self.mapper.model_label2id.get("O"))
                     
                     # Tracciamento intervento LLM
                     intervention = {
                           'token': token_text,
                     ...
                     if corrected_id != pred_id:
                           ...
                           if corrected_id == labels_flat[idx] and pred_id != labels_flat[idx]:
                              ...
                              # LLM ha peggiorato una predizione corretta
                              res.llm_wrong_corrections += 1
                              res.per_class_stats[true_label]['llm_hurt'] += 1
                  
                  # Traccia se hybrid è corretto
      ...
      logger.info(f"Evaluation complete. LLM calls: {res.llm_calls}, Helpful: {res.llm_corrections}, Harmful: {res.llm_wrong_corrections}")
   ...
   def _generate_comparison_report(self, res: HybridResult):
      ...
      base_report = classification_report(y_true, y_base, labels=labels, zero_division=0)
      hyb_report = classification_report(y_true, y_hyb, labels=labels, zero_division=0)
      ...
      report_lines.append(f"Logic: (Conf < {self.config.THRESHOLD_CONF}) OR (Entropy > {self.config.THRESHOLD_ENTROPY})")
      ...
      report_lines.append(f"Net Improvement: {res.llm_corrections - res.llm_wrong_corrections}")
      report_lines.append("=" * 80)
      ...
      for label in sorted(res.per_class_stats.keys()):
         ...
         report_lines.append(f"  Helped: {stats['llm_helped']}, Hurt: {stats['llm_hurt']}")
      ...
      with open(os.path.join(self.config.output_dir, "comparison_report.txt"), "w") as f:
         f.write(report_text)
...
if __name__ == "__main__":
   ...
   if os.getenv("OPENAI_API_KEY"):
      ...
      router = OptimizedLLMRouter(source="openai", model="gpt-4o-mini", enable_cache=True)
   else:
      logger.warning("No OPENAI_API_KEY found. Trying Ollama...")
      try:
         router = OptimizedLLMRouter(source="ollama", ollama_model="qwen2.5:3b", enable_cache=True)
      except Exception as e:
