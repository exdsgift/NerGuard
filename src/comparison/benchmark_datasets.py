...
NVIDIA_TO_MODEL_MAP = {
    ...
    "tax_id": "TAXNUM",
    "driver_license": "DRIVERLICENSENUM", "drivers_license": "DRIVERLICENSENUM",
    "credit_debit_card": "CREDITCARDNUMBER",
    "date": "DATE", "date_of_birth": "DATE", "time": "TIME",
    "age": "AGE", "gender": "GENDER", "sexuality": "GENDER",
    # Approximations
    "street_address": "STREET",
    # Excluded (Mapped to O)
    "company_name": "O", "organization": "O", "occupation": "O",
    "url": "O", "ip_address": "O", "ipv4": "O", "ipv6": "O",
    "country": "O", "state": "O", "account_number": "O",
    "national_id": "O", "passport_number": "O",
}
...
class LabelMapper:
    """Handles mapping between Nvidia PII labels and Model specific labels."""
    def __init__(self, model_id2label: Dict[int, str]):
        ...
        self.map_counter = {k: 0 for k in NVIDIA_TO_MODEL_MAP.keys()}

    def get_token_label_id(self, nv_label: str, is_start: bool) -> int:
        target_base = NVIDIA_TO_MODEL_MAP.get(nv_label, "O")
        
        if nv_label in self.map_counter:
            self.map_counter[nv_label] += 1
        ...
        # Fallback to O if the specific B-TYPE/I-TYPE is missing in model
...
class NvidiaEvaluator:
    ...
    def _generate_report(self, true_ids: List[int], pred_ids: List[int]):
        ...
        cls_report = classification_report(y_true, y_pred, labels=labels_no_o, digits=4, zero_division=0)
        logger.info("\n" + cls_report)
        ...
        sorted_stats = sorted(self.mapper.map_counter.items(), key=lambda x: x[1], reverse=True)
    ...
    def _plot_cm(self, y_true: List[str], y_pred: List[str]):
        ...
        sns.heatmap(cm_norm, annot=True, fmt='.2f', xticklabels=relevant_labels, yticklabels=relevant_labels, cmap='Blues')
