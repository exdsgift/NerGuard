"""WikiNER Hybrid system — bert-base-NER + LLM uncertainty routing on WikiNeural.

General NER compatibility experiment (thesis supervisor suggestion iv):
  Uses PassthroughValidator (no PII-specific structural rules) to test whether
  selective routing via entropy/confidence gating helps on standard NER labels
  (PER, ORG, LOC, MISC) without any domain-specific engineering.

This is the key experiment to close the over-engineering critique: if routing
still improves F1 with a compact general label schema and no domain-specific
validators, then selective routing is a general contribution, not a PII artifact.
"""
...
from src.tasks.wikiner.config import WIKINER_MODEL, get_wikiner_route_config
...
class WikiNERHybrid(SystemWrapper):
    def __init__(
        self,
        model_path: str = WIKINER_MODEL,
        device: str = "auto",
    ...
    def name(self) -> str:
        return f"WikiNER Hybrid ({self.llm_model})"
    ...
    def setup(self) -> None:
        ...
        from src.tasks.wikiner.prompts import WikinerPromptProvider

        logger.info(f"Loading WikiNER Hybrid (model={self.model_path}, LLM={self.llm_model})")
        ...
        # Generic NER prompt — no PII-specific instructions
        prompt_provider = WikinerPromptProvider()
        ...
        # General routing config — no regex validators, no entity-specific overrides
        route_config = get_wikiner_route_config()
        self.entity_router = EntitySpecificRouter.from_config(route_config)

        self._routing_meta = self._empty_routing_meta()
        logger.info(f"WikiNER Hybrid ready on {self.device}")
    ...
    def calibrate_thresholds(self, samples: list) -> None:
        ...
        result, calibrated_config = calibrator.calibrate(
            ...
            device=self.device,
            samples=samples,
            base_config=get_wikiner_route_config(),
        )
    ...
    def predict(
        ...
        entity_classes_str = _json.dumps(sorted(self._native_labels | {"O"}), indent=2)
        llm_calls = 0
    ...
    def predict_ner_only(
        ...
        """Phase 1: NER inference only, collect pending spans for batch LLM."""
        from src.inference.span_assembler import assemble_entity_spans
    ...
    async def resolve_routing_batch(
        ...
        """Phase 2: Batch LLM calls, apply corrections."""
        import asyncio
        ...
        logger.info(f"Batch routing: {len(tasks)} LLM calls across {len(deferred_preds)} samples")
