"""WikiNER Base system — dslim/bert-base-NER on WikiNeural, no LLM routing.

General NER compatibility experiment (thesis supervisor suggestion iv):
  Disable PII-specific validators, use compact label schema PER/ORG/LOC/MISC,
  run base model only. This is the baseline for the routing ablation.
"""
...
from src.tasks.wikiner.config import WIKINER_MODEL
...
class WikiNERBase(SystemWrapper):
    def __init__(self, model_path: str = WIKINER_MODEL, device: str = "auto"):
    ...
    def name(self) -> str:
        return "WikiNER Base (bert-base-NER)"
    ...
    def setup(self) -> None:
        logger.info(f"Loading WikiNER base model from {self.model_path}")
        ...
        logger.info(f"WikiNER Base ready on {self.device} ({len(self._native_labels)} entity types)")
