"""Financial Hybrid system — BERT BUSTER + LLM uncertainty routing.

Demonstrates the NerGuard framework on financial NER: 6 M&A entity types
(buying/selling/acquired companies, advisors, revenues) from SEC filings.
Uses PassthroughValidator since there are no structural regex patterns.
"""
...
from src.tasks.financial.config import BUSTER_MODEL, get_financial_route_config
...
class FinancialHybrid(SystemWrapper):
    def __init__(
        self,
        model_path: str = BUSTER_MODEL,
        device: str = "auto",
    ...
    def name(self) -> str:
        return f"Financial Hybrid ({self.llm_model})"

    def native_labels(self) -> Set[str]:
        if self._native_labels is not None:
            return self._native_labels
        from src.tasks.financial.config import BUSTER_ENTITY_CLASSES
    ...
    def setup(self) -> None:
        ...
        from src.tasks.financial.prompts import FinancialPromptProvider

        logger.info(f"Loading Financial Hybrid (LLM: {self.llm_source}/{self.llm_model})")
        ...
        prompt_provider = FinancialPromptProvider()
        ...
        route_config = get_financial_route_config()
        self.entity_router = EntitySpecificRouter.from_config(route_config)

        self._routing_meta = self._empty_routing_meta()
        logger.info(f"Financial Hybrid ready on {self.device}")
    ...
    def calibrate_thresholds(self, samples: list) -> None:
        ...
        result, calibrated_config = calibrator.calibrate(
            ...
            device=self.device,
            samples=samples,
            base_config=get_financial_route_config(),
        )
