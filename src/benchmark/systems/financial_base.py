"""Financial NER base system wrapper — BERT model on BUSTER (M&A), no LLM routing."""
...
from src.tasks.financial.config import BUSTER_MODEL
...
class FinancialBase(SystemWrapper):
    def __init__(self, model_path: str = BUSTER_MODEL, device: str = "auto"):
    ...
    def setup(self) -> None:
        logger.info(f"Loading financial model from {self.model_path}")
        ...
        logger.info(f"Financial Base ready on {self.device} ({len(self._native_labels)} entity types)")
