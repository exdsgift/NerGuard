"""Biomedical Hybrid system — RoBERTa BC5CDR + LLM uncertainty routing.

Demonstrates the NerGuard framework's generalizability: same routing
pipeline (entropy-based, span-level, anchor propagation) applied to
biomedical NER instead of PII detection. Uses PassthroughValidator
since there are no structural regex patterns for Chemical/Disease.
"""
...
from src.tasks.biomedical.config import BC5CDR_MODEL, get_biomedical_route_config
...
class BiomedicalHybrid(SystemWrapper):
    def __init__(
        self,
        model_path: str = BC5CDR_MODEL,
        device: str = "auto",
    ...
    def name(self) -> str:
        return f"Biomedical Hybrid ({self.llm_model})"
    ...
    def setup(self) -> None:
        ...
        from src.tasks.biomedical.prompts import BiomedicalPromptProvider

        logger.info(f"Loading Biomedical Hybrid (LLM: {self.llm_source}/{self.llm_model})")
        ...
        # Biomedical-specific prompt provider
        prompt_provider = BiomedicalPromptProvider()

        # LLM router with biomedical prompts
        ...
        # Entity router from biomedical config
        route_config = get_biomedical_route_config()
        self.entity_router = EntitySpecificRouter.from_config(route_config)

        self._routing_meta = self._empty_routing_meta()
        logger.info(f"Biomedical Hybrid ready on {self.device}")
    ...
    def predict(
        ...
        # Span-level LLM routing with anchor propagation
        ...
        import json
        entity_classes_str = json.dumps(sorted(self._native_labels | {"O"}), indent=2)
        llm_calls = 0
