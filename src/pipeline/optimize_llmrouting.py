""" Optimized LLM routing with caching and error handling """
...
class LLMCache:
    ...
    def _make_key(
        self, target: str, context: str, prev_label: str, current_pred: str
    ) -> str:
...
class OptimizedLLMRouter:
    ...
    def disambiguate(
        ...
        if self.cache:
            cached_result = self.cache.get(
                clean_token, context, prev_label, current_pred
            )
            if cached_result:
        ...
        try:
            ...
            if self.cache:
                self.cache.set(
                    clean_token, context, prev_label, current_pred, validated_result
                )
    ...
    def _validate_response(
        self, raw: Dict[str, Any], fallback_label: str
    ) -> Dict[str, Any]:
        ...
        if label not in self.valid_labels:
            print(
                f"   [WARNING] Invalid label '{label}' → fallback to '{fallback_label}'"
            )
