...
# Ensure seaborn is installed for better styling defaults, 
# falling back to matplotlib if not available.
try:
   import seaborn as sns
...
# --- IMPORT CORRETTO DAL FILE FORNITO ---
try:
...
class PlottingMixin:
   ...
   # Academic Color Palette - Clean, colorblind-friendly with high contrast
   COLORS = {
      'baseline': '#1f77b4',      # Blue (matches Blues colormap)
      'hybrid': '#2ca02c',        # Green (matches Greens colormap)
      'positive': '#2ca02c',      # Green for improvements
      'negative': '#d62728',      # Red for degradations
      'neutral': '#7f7f7f',       # Grey
   ...
   def _plot_per_class_improvements(self, res: Any):
      ...
      ax1.bar(x - width/2, df['baseline'], width, label='Baseline',
               color=self.COLORS['baseline'], alpha=0.8, zorder=3, edgecolor='white', linewidth=1)
      ax1.bar(x + width/2, df['hybrid'], width, label='Hybrid',
               color=self.COLORS['hybrid'], alpha=0.8, zorder=3, edgecolor='white', linewidth=1)
      ...
      ax1.axhline(y=50, color=self.COLORS['neutral'], linestyle='--', linewidth=1, alpha=0.5)
      ax1.set_ylim(bottom=0)

      # 2. Improvement (Diverging Bar Chart)
      colors = [self.COLORS['positive'] if x >= 0 else self.COLORS['negative'] for x in df['improvement']]
      ax2.bar(x, df['improvement'], color=colors, alpha=0.8, zorder=3, edgecolor='white', linewidth=1)

      self._style_axis(ax2, 'Net Accuracy Improvement (Hybrid - Baseline)', 'Entity Class', 'Improvement (pp)')
      ax2.set_xticks(x)
   ...
   def _plot_confidence_entropy_analysis(self, res: Any):
      ...
      axes[0, 0].hist(res.confidence_scores, bins=40, alpha=0.8,
                     color=self.COLORS['baseline'], edgecolor='white', linewidth=1, zorder=3)
      axes[0, 0].axvline(self.config.THRESHOLD_CONF, color=self.COLORS['negative'], linestyle='--',
                        linewidth=2, label=f'Threshold ({self.config.THRESHOLD_CONF})')
      self._style_axis(axes[0, 0], 'Confidence Score Distribution', 'Confidence Score', 'Count')
      axes[0, 0].legend(frameon=True, fontsize=11, edgecolor='#333333')

      # 2. Entropy Distribution
      axes[0, 1].hist(res.entropy_scores, bins=40, alpha=0.8,
                     color=self.COLORS['accent_1'], edgecolor='white', linewidth=1, zorder=3)
      axes[0, 1].axvline(self.config.THRESHOLD_ENTROPY, color=self.COLORS['negative'], linestyle='--',
                        linewidth=2, label=f'Threshold ({self.config.THRESHOLD_ENTROPY})')
      self._style_axis(axes[0, 1], 'Entropy Score Distribution', 'Entropy Score', 'Count')
      ...
      if res.llm_interventions:
         ...
         axes[1, 0].scatter([x['confidence'] for x in helped], [x['entropy'] for x in helped],
                              c=self.COLORS['positive'], alpha=0.6, s=50, label='LLM Helped', edgecolors='white', linewidth=1)
         axes[1, 0].scatter([x['confidence'] for x in hurt], [x['entropy'] for x in hurt],
                              c=self.COLORS['negative'], alpha=0.6, s=50, label='LLM Hurt', edgecolors='white', linewidth=1)

         self._style_axis(axes[1, 0], 'Intervention Quality vs. Uncertainty', 'Confidence', 'Entropy')
         ...
         axes[1, 0].legend(loc='upper right', frameon=True, fontsize=11, edgecolor='#333333')
      ...
      if res.llm_interventions:
         ...
         axes[1, 1].bar(x - width/2, q_stats['helped'], width, label='Helped',
                       color=self.COLORS['positive'], alpha=0.8, edgecolor='white', linewidth=1)
         axes[1, 1].bar(x + width/2, q_stats['hurt'], width, label='Hurt',
                       color=self.COLORS['negative'], alpha=0.8, edgecolor='white', linewidth=1)

         self._style_axis(axes[1, 1], 'LLM Impact by Confidence Quartile', 'Confidence Quartile', 'Count')
   ...
   def _plot_llm_impact_analysis(self, res: Any):
      ...
      if counts:
         ...
         bars = axes[1, 0].barh(y_pos, list(top_classes.values()), color=self.COLORS['baseline'],
      ...
      if net_impact:
         ...
         colors = [self.COLORS['positive'] if v > 0 else self.COLORS['negative'] for v in sorted_imp.values()]
         y_pos = np.arange(len(sorted_imp))

         bars = axes[1, 1].barh(y_pos, list(sorted_imp.values()), color=colors,
                        alpha=0.8, edgecolor='white', linewidth=1)
         axes[1, 1].set_yticks(y_pos)
         axes[1, 1].set_yticklabels(sorted_imp.keys(), fontsize=10)
         axes[1, 1].axvline(0, color='#333333', linewidth=1)
         axes[1, 1].invert_yaxis()
         self._style_axis(axes[1, 1], 'Top 10 Classes: Net Correction Impact', 'Net Change', '')
   ...
   def _plot_error_analysis(self, y_true: List, y_base: List, y_hyb: List):
      ...
      for t, b, h in zip(y_true, y_base, y_hyb):
         ...
         if b_wrong and not h_wrong: fixed_errors.append(t)
         if h_wrong and not b_wrong: new_errors.append(t)
         if b_wrong: error_types['baseline_only'].append(t)
         if h_wrong: error_types['hybrid_only'].append(t)
      ...
      axes[0, 0].bar(x - width/2, [base_counts[c] for c in target_classes], width,
                     label='Baseline', color=self.COLORS['baseline'], alpha=0.8,
                     edgecolor='white', linewidth=1)
      axes[0, 0].bar(x + width/2, [hyb_counts[c] for c in target_classes], width,
                     label='Hybrid', color=self.COLORS['hybrid'], alpha=0.8,
                     edgecolor='white', linewidth=1)
      ...
      changed_classes = sorted(list(set(list(fixed_c.keys()) + list(new_c.keys()))),
                              key=lambda k: fixed_c[k] - new_c[k], reverse=True)[:12]

      if changed_classes:
         x = np.arange(len(changed_classes))
         axes[0, 1].bar(x, [fixed_c[c] for c in changed_classes],
                        label='Fixed', color=self.COLORS['positive'], alpha=0.8,
                        edgecolor='white', linewidth=1)
         axes[0, 1].bar(x, [-new_c[c] for c in changed_classes],
                        label='New Error', color=self.COLORS['negative'], alpha=0.8,
                        edgecolor='white', linewidth=1)

         self._style_axis(axes[0, 1], 'Error Flux: Fixed vs. Introduced', '', 'Count')
         axes[0, 1].set_xticks(x)
         axes[0, 1].set_xticklabels(changed_classes, rotation=45, ha='right', fontsize=10)
      ...
      colors_m = [self.COLORS['baseline'], self.COLORS['hybrid'], self.COLORS['positive'], self.COLORS['negative']]
      ...
      if err_rates:
         x = np.arange(len(target_classes))
         axes[1, 1].bar(x - width/2, [err_rates[c]['base'] for c in target_classes], width,
                        label='Baseline', color=self.COLORS['baseline'], alpha=0.8,
                        edgecolor='white', linewidth=1)
         axes[1, 1].bar(x + width/2, [err_rates[c]['hyb'] for c in target_classes], width,
                        label='Hybrid', color=self.COLORS['hybrid'], alpha=0.8,
                        edgecolor='white', linewidth=1)

         self._style_axis(axes[1, 1], 'Error Rate Percentage', '', 'Error Rate (%)')
         axes[1, 1].set_xticks(x)
         axes[1, 1].set_xticklabels(target_classes, rotation=45, ha='right', fontsize=10)
         axes[1, 1].legend(frameon=True, fontsize=11, edgecolor='#333333')
   ...
   def _plot_confusion_matrices(self, y_true, y_base, y_hyb, labels):
      ...
      def plot_ax(y_t, y_p, axis, title, cmap):
         ...
         sns.heatmap(cm_norm, annot=False, fmt='.2f', xticklabels=labels,
                     yticklabels=labels, cmap=cmap, ax=axis, cbar_kws={'shrink': 0.85})
