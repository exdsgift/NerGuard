...
def predict_and_redact(text: str, confidence_threshold: float = CONFIDENCE_THRESHOLD):
    ...
    for pred_id, confidence, offset in zip(predictions, confidences, offset_mapping):
        ...
        if label != "O":
            ...
            if label.startswith("B-") or not current_entity or entity_type != current_entity["type"]:
                if current_entity:
    ...
    return {
        "original": text,
        "redacted": redacted,
        "entities": [(text[e["start"]:e["end"]], e["type"], e["confidence"]) 
                     for e in entities]
    }
...
def test(text: str, title: str = "", threshold: float = CONFIDENCE_THRESHOLD):
    ...
    if result['entities']:
        ...
        for entity_text, entity_type, conf in result['entities'][:10]:  # Max 10
            display_text = entity_text[:30] + "..." if len(entity_text) > 30 else entity_text
...
""".strip(), "Domanda di Finanziamento")
...
def interactive():
    ...
    while True:
        try:
            ...
            thresh_input = input(f"⚙️  Threshold (default {CONFIDENCE_THRESHOLD}): ").strip()
