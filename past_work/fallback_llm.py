...
ner_pipeline = pipeline(
   "ner", model=deberta_model, tokenizer=tokenizer, aggregation_strategy="simple", device=0,
)
...
class PIIVerification(BaseModel):
   ...
   corrected_type: Optional[str] = Field(
      description="Predicted type correct or incorrect.", default=None
   )
   confidence_score: float = Field(
      description="Test confidence intervall (0-1)",
      ge=0.0,
      le=1.0,
   )
   reasoning: str = Field(description="Explenation, Lang: EN")

# todo: Config LLM file in a different file

CONFIDENCE_THRESHOLD = 0.50  # * Modify this!
OLLAMA_MODEL = "qwen2.5:3b" #llama3.2:3b
PII_TYPES = {
...
def extract_entities_with_deberta(text: str):
   ...
   for chunked_text, token_offset in chunk_text(str(text)):
      ner_results = ner_pipeline(chunked_text)
      for res in ner_results:
         chunk_start = text.find(chunked_text)
         if chunk_start == -1:
            chunk_start = 0
         actual_start = chunk_start + res["start"]
         actual_end = chunk_start + res["end"]
         res_info = {
               "text": res["word"],
               ...
               "end": actual_end,
               "context_window": text[
                  max(0, actual_start - 50) : min(len(text), actual_end + 50)
               ],
         }
         if res["score"] < CONFIDENCE_THRESHOLD:
               uncertain_entities.append(res_info)
         else:
               confident_entities.append(res_info)
...
def create_verification_prompt(entity: dict) -> str:
   """
   ...
   json_example = {
      ...
      "reasoning": "The entity is clearly a person's name in context"
   }
   ...
   4. If the proper type is not in the provided list, choose the closest type from the list!

   Respond in JSON format following exactly this schema:
   {json.dumps(json_example, indent=2)}

   Rules:
   - is_valid_pii: true if it's PII, false otherwise
   - corrected_type: null if type is correct, otherwise provide the correct type from the list
...
def verify_with_llama_local(entity: dict, text: str) -> dict:
   ...
   """
   prompt = create_verification_prompt(entity)
   ...
   try:
      ...
      response = ollama.chat(
         ...
         format=PIIVerification.model_json_schema(),
         options={
               "temperature": 0.3,
               "num_predict": 500,
         },
      ...
      print(f"Error parsing JSON LLM: {e}")
      # Fallback
      return {
         "is_valid_pii": True,
         "corrected_type": None,
         "confidence_score": entity["confidence"],
         "reasoning": "ERROR in generating validation, kept original prediction",
      }
...
def pii_detection_pipeline(text: str, verify_uncertain: bool = True) -> dict:
   ...
   print(f"\nDecoder elaboration time: {end_time_enc:.2}s")
   ...
   if verify_uncertain and uncertain_entities:
      ...
      for i, uncertain_entity in enumerate(uncertain_entities, 1):
         ...
         print(f"LLM elaboration time: {end_time_llm}")
...
def redact_text(text: str, entities: List[dict], replacement_char="█") -> dict:
   """
   ...
   for entity in sorted_entities:
      escaped_text = re.escape(entity["text"])
      ...
      redaction_map.append(
         {
               ...
               "start": entity["start"],
               "end": entity["end"],
               "redacted_with": (len(entity["text"]) * replacement_char),
         }
      )
...
def redact_text_compact(text: str, entities: List[dict], replacement_char="█") -> dict:
   """
   ...
   """
   REDACTION_SYMBOL = replacement_char * 5  # Fixed length redaction
...
def pii_detection_with_redaction(
   text: str, verify_uncertain: bool = True, redaction_style: str = "length_match"
) -> dict:
   """
...
"""
   results = pii_detection_with_redaction(
      sample_text,
      verify_uncertain=True,
      redaction_style="compact",
   )
